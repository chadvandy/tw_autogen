...
function DocObj:read(line)
    if self.section == 0 then
        if line:find("<div id=\"content\">") then
            ...
            local class_check = string.format("<a href=\"#class:%s\">", wild_un)
            local fn_check = string.format("<a href=\"#function:%s:%s\">", self.name, wild_un)
            --- save the class name for this object
            if line:find(class_check) then
                local sx,ex = line:find(class_check)
                local rep = line:sub(sx, ex)
                rep = rep:gsub("<a href=\"#class:", "")
                rep = rep:gsub("\">", "")

                self.name = rep
                print("Class internal name is " .. rep)
            end
            ...
            if line:find(fn_check) then 
                ...
                local rep = line:sub(sx, ex)
                rep = rep:gsub("<a href=\"#function:"..self.name..":", "")
                rep = rep:gsub("\">", "")

                self.methods[rep] = MethodObj:new(rep)

                printf("Found function %s:%s()", self.name, rep)
            elseif line:find("href=\"#self.section") then
                -- meh
            end
        end
    elseif self.section >= 1 then
        local fn_name = string.format("<a name=\"function:%s:%s\">", self.name, wild_un)
        ...
        elseif self.section == 1.1 then -- getting a specific function or its details!
            if line:find(fn_name) then
                local sx,ex = line:find(fn_name)
                local rep = line:sub(sx, ex)
                rep = rep:gsub("<a name=\"function:"..self.name..":", "")
                rep = rep:gsub("\">", "")
                ...
                printf("In %s:%s()", self.name, self.this_function.name)
                ...
                for str in string.gmatch(line, "<i class=\"parameter\">"..wild.."</i>") do
                    ...
                    str = str:gsub(" ", "_")
                    str = str:gsub("%.", "")
                    print("FOUND PARAM NAME: " .. str .. " at index " .. #self.methods[self.this_function.name].params+1)

                    ---@class param
                    local param = TypeObj:new(str)

                    self.methods[self.this_function.name].params[#self.methods[self.this_function.name].params+1] = param
                end
            elseif self.this_function then
                ...
                elseif self.this_function.next == 1 then -- We're in desc!
                    ...
                    elseif is then
                        ...
                        -- s = s .. "\n" .. d
                        printf("Description of %s:%s() is %q", self.name, self.this_function.name, d)
                        self.methods[self.this_function.name].desc[#self.methods[self.this_function.name].desc+1] = d
                    end
                elseif self.this_function.next == 2 then -- we're in Params!
                    ...
                    if line:find("<strong>") then 
                        ...
                        self.current_param = self.methods[self.this_function.name].params[current_order]
                ...
                elseif self.this_function.next == 3 then -- we're in Returns!
                    ...
                    if line:find("<li><code>") then
                        ...
                        if t then
                            ...
                            printf("Trying to get function %s:%s()", self.name, self.this_function.name)
                            local method = self.methods[self.this_function.name]
...
function TypeObj:print(is_param)
    ...
    if is_param then
        if corrected_type == "..." then
            ...
            insf("---@param %s %s %s", name, corrected_type, desc)
        end
    else
