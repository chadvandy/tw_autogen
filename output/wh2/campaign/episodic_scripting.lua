...
--- 
---@param sound_event Triggers #campaign voiceover audio at a character's 3D position. The character specified also partially specifies the path by which the voiceover sound is looked up.
---@param character_lookup string #Sound event name.
---@param delay string #Character lookup string - see Character Lookups for more information.
...
---@param name string #Unique name for this composite scene, by which it may later be removed with cm:remove_scripted_composite_scene.
---@param composite_scene string #Composite scene key from the campaign_composite_scenes table.
---@param settlement_key string #Key of settlement to add the scene to, from the campaign_map_settlements table.
---@param facing_x number #Logical x co-ordinate of a position this composite scene faces.
---@param facing_y number #Logical y co-ordinate of a position this composite scene faces.
---@param one_shot boolean #One shot - if set to true, this composite scene is not added to the internal list of scenes and can't later be removed with cm:remove_scripted_composite_scene. However, the name of one-shot scenes does not have to be unique.
---@param show_in_seen_shroud boolean #Sets whether this composite scene should be drawn when in thin shroud over previously-seen terrain.
...
function episodic_scripting:add_scripted_composite_scene_to_settlement(name, composite_scene, settlement_key, facing_x, facing_y, one_shot, show_in_seen_shroud, show_in_unseen_shroud) end
...
---@param show_in_unseen_shroud boolean #Sets whether this composite scene should be drawn when in thick shroud over unseen terrain.
---@return boolean #action successful 
function episodic_scripting:add_scripted_composite_scene_to_settlement_port(name, composite_scene, settlement_key, facing_x, facing_y, one_shot, show_in_seen_shroud, show_in_unseen_shroud) end
...
---@param command_queue boolean #Send this command via the command queue. Setting this to true will cause a delay of a game tick or two before the force is created.
function episodic_scripting:create_force(faction_key, unit_list, region_key, x, y, id, exclude_unique_characters, command_queue) end
...
---@param exclude_unique_characters boolean #Prevent this force from having a unique character appointed as its general.
---@param command_queue boolean #Send this command via the command queue. Setting this to true will cause a delay of a game tick or two before the force is created.
function episodic_scripting:create_force_with_full_diplomatic_discovery(faction_key, unit_list, region_key, x, y, id, exclude_unique_characters, command_queue) end
...
---@param command_queue boolean #Send this command via the command queue. Setting this to true will cause a delay of a game tick or two before the force is created.
function episodic_scripting:create_agent(faction_key, agent_type, agent_subtype, x, y, id, command_queue) end
...
---@param character_lookup string #Character lookup string of subject character. For more information see the documentation on Character Lookups.
---@param mf_cqi number #Command-queue index of the military force to appoint the character to.
function episodic_scripting:appoint_character_to_force(character_lookup, mf_cqi) end
...
---@param character FAMILY_MEMBER_SCRIPT_INTERFACE #FAMILY_MEMBER_SCRIPT_INTERFACE related to the character to apply experience points to.
---@param points number #Experience points to award.
function episodic_scripting:add_agent_experience_through_family_member(character, points) end
...
---@param command_queue boolean #Send the order via the command queue. This causes a short delay before the order is actually given.
function episodic_scripting:move_to(character_lookup, x, y, command_queue) end
...
---@param y number #Logical y co-ordinate of target position.
---@param command_queue boolean #Send the order via the command queue. This causes a short delay before the order is actually given.
function episodic_scripting:teleport_to(character_lookup, x, y, command_queue) end
...
---@param x number #Logical x co-ordinate of target position.
---@param y number #Logical y co-ordinate of target position.
function episodic_scripting:leave_garrison(character_lookup, x, y) end
...
---@param lay_siege boolean #Instructs the character to lay siege to the target. This is useful if the target character is in a garrison.
function episodic_scripting:attack(character_lookup, target_character_lookup, lay_siege) end
...
---@param target_character_lookup string #Target character lookup string. For more information, see Character Lookups.
---@param lay_siege boolean #Instructs the character to lay siege to the target. This is useful if the target character is in a garrison.
function episodic_scripting:attack_queued(character_lookup, target_character_lookup, lay_siege) end
...
---@param command_queue boolean #Send the order via the command queue. This causes a short delay before the order is actually given.
function episodic_scripting:attack_region(character_lookup, region_key, command_queue) end
...
---@param command_queue boolean #Send the order via the command queue. This causes a short delay before the order is actually given.
function episodic_scripting:seek_exchange(character_lookup, target_character_lookup, show_ui, command_queue) end
...
---@param command_queue boolean #Send the order via the command queue. This causes a short delay before the order is actually given.
function episodic_scripting:kill_character(character_lookup, destroy_force, command_queue) end

--- Kills a specified character and their associated unit, and optionally also the entire military force they command.
---@param character_lookup string #Character lookup string. For more information, see Character Lookups.
---@param destroy_force boolean #Destroy the character and the entire military force they command.
---@param command_queue boolean #Send the order via the command queue. This causes a short delay before the order is actually given.
function episodic_scripting:kill_character_and_commanded_unit(character_lookup, destroy_force, command_queue) end
...
---@param command_queue boolean #Send the order via the command queue. This causes a short delay before the order is actually given.
function episodic_scripting:wound_character(character_lookup, convalescence_time, command_queue) end
...
---@param command_queue boolean? #optional, default value=true Issue this command via the command queue.
function episodic_scripting:force_add_trait(character_lookup, trait_key, trait_points, command_queue) end
...
---@param faction_key string #Faction key, from the factions table.
---@param amount number #Treasury modification.
...
---@param xp number #The experience level of the units when recruited. This should be an integer value between 0-9.
...
function episodic_scripting:add_unit_to_faction_mercenary_pool(faction, unit, count, replenishment_chance, max_units, max_per_turn, xp, faction_restriction, subculture_restriction, tech_restriction, partial_replenishment) end
...
---@param subculture_restriction string #The key of the subculture who can actually recruit the units, from the cultures_subcultures database table. An empty string "" may be supplied to omit this, which will usually be the case.
---@param tech_restriction string #The key of a technology that must be researched in order to recruit the units, from the technologies database table.
function episodic_scripting:add_unit_to_province_mercenary_pool(region, unit, count, replenishment_chance, max_units, max_per_turn, xp, faction_restriction, subculture_restriction, tech_restriction) end
...
function episodic_scripting:trigger_mission(faction_key, mission_key, fire_immediately, use_command_queue) end
...
---@param use_command_queue boolean? #optional, default value=false Trigger the mission via the command queue or not.
function episodic_scripting:trigger_custom_mission(faction_key, mission_key, use_command_queue) end
...
---@param use_command_queue boolean? #optional, default value=false Trigger the mission via the command queue or not.
function episodic_scripting:trigger_custom_mission_from_string(faction_key, mission, use_command_queue) end

--- Cancels an active custom mission.
---@param faction_key string #Faction key, from the factions table.
---@param mission_key string #Mission key, from the missions table.
---@param use_command_queue boolean? #optional, default value=false Sends the command via the command queue or not.
function episodic_scripting:cancel_custom_mission(faction_key, mission_key, use_command_queue) end
...
function episodic_scripting:complete_scripted_mission_objective(mission_key, script_key, is_success) end
...
---@param subcategory string #Event feed subcategory to block, from the event_feed_events table. Supply a blank string to not filter by event.
function episodic_scripting:disable_event_feed_events(should_disable, category, subcategory, subcategory) end
...
---@param slot SLOT_SCRIPT_INTERFACE #Slot script interface.
...
---@param turns number #Number of turns to apply the effect bundle for. -1 may be supplied to apply the effect indefinitely.
---@param command_queue boolean #Send this command via the command queue.
function episodic_scripting:apply_effect_bundle_to_characters_force(effect_bundle_key, character_cqi, turns, command_queue) end
...
---@param use_command_queue boolean? #optional, default value=true Sends the declaration command via the command queue.
function episodic_scripting:force_declare_war(attacking_faction_key, target_faction_key, invite_attacker_allies, invite_defender_allies, use_command_queue) end
...
---@param interface_ritual_setup RITUAL_SETUP_SCRIPT_INTERFACE #Ritual setup to use. This can be either a RITUAL_SETUP_SCRIPT_INTERFACE or a MODIFY_RITUAL_SETUP_SCRIPT_INTERFACE object.
function episodic_scripting:perform_ritual_with_setup(interface_ritual_setup) end
...
---@param settlement_key string #Settlement key, from the campaign_map_settlements database table. This will be settlement: followed by the region key.
